...
public class AdminMenuSDAssetsController extends Connection {
    ...
    @FXML
    void removeAssets(ActionEvent event) {
        dataObject = new DataObject();
        if (socket == null || socket.isClosed()) {
            connect();
        }
        try {
            dataObject.setCommand("deleteAssets");
            dataObject.setId(showTable.getSelectionModel().getSelectedItem().getId());
